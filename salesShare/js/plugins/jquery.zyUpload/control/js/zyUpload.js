	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					/* 提供给外部的接口方法 */
					onSelect         : function(selectFiles, files){debugger},// 选择文件的回调方法  selectFile:当前选中的文件  allFiles:还没上传的全部文件
					onDelete		 : function(file, files){debugger},     // 删除一个文件的回调方法 file:当前删除的文件  files:删除之后的文件
					onSuccess		 : function(file){debugger},            // 文件上传成功的回调方法
					onFailure		 : function(file){debugger},            // 文件上传失败的回调方法
					onComplete		 : function(responseInfo){debugger},    // 上传完成的回调方法
			};
